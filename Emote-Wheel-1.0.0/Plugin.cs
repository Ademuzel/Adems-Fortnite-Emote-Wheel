    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        public static AudioClip LoadSoundFromResource(string resourcePath)
        {
            ...
            if (!audioPool.ContainsKey(resourcePath))
            {
                ...
                if (stream != null)
                {
                    ...
                    // Type-filtered lookup: the bundle can contain multiple assets
                    // that share the same name but differ in type (e.g. an
                    // AnimationClip and an AudioClip both named "zany"). The
                    // untyped LoadAsset(string) overload ignores type and just
                    // returns the first name match, which can silently resolve
                    // to the wrong asset. LoadAsset<AudioClip> filters by type
                    // so it can only ever return a real AudioClip or null.
                    sound = assetBundle.LoadAsset<AudioClip>(resourcePath);

                    if (sound == null)
                    {
                        UnityEngine.Object rawAsset = assetBundle.LoadAsset(resourcePath);
                        if (rawAsset == null)
                            Debug.LogError($"[FortniteEmoteWheel] AssetBundle has no asset named \"{resourcePath}\" — check the asset's name inside the bundle matches this string exactly (case included).");
                        else
                            Debug.LogError($"[FortniteEmoteWheel] Asset \"{resourcePath}\" exists but as a {rawAsset.GetType().Name}, not an AudioClip — likely a duplicate name shared with a non-audio asset in the bundle.");
                    }

                    // Only cache successful loads — caching null would permanently
                    // hide this asset for the rest of the session, even after a fix.
                    if (sound != null)
                        audioPool.Add(resourcePath, sound);
                    else
                        Debug.LogError($"[FortniteEmoteWheel] Failed to resolve AudioClip \"{resourcePath}\" — this sound will not play until fixed.");
                }
                else
                {
                    Debug.LogError("[FortniteEmoteWheel] Failed to load sound from resource: " + resourcePath);
                }
            }
            else
        ...
        public void Update()
        {
            ...
            if (Classes.Wheel.instance == null && VRRig.LocalRig != null)
            {
                ...
                // DIAGNOSTIC: force this object and every child onto the same
                // layer as the player's own body. The body is known to render
                // correctly in both eyes, so if this fixes the one-eye issue,
                // it confirms the Wheel's original layer was excluded from one
                // eye camera's culling mask. If it does NOT fix it, layers are
                // ruled out entirely as the cause.
                SetLayerRecursively(Wheel, VRRig.LocalRig.gameObject.layer);
